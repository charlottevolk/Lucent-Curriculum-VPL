...
class LayerNorm2d(nn.LayerNorm):
    def forward(self, x1):
        ...
        x1 = F.layer_norm(x1, self.normalized_shape, self.weight, self.bias, self.eps)
...
class CNBlock(nn.Module):
    ...
    def forward(self, x1):
        result = self.layer_scale * self.block(x1)
...
class ConvNeXt(nn.Module):
    def __init__(self, in_chans=3, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], layer_scale=1e-6):
    ...
    def extract_skip_features(self, x1):
        ...
        for layer in self.features:
            x1 = layer(x1)
            skip_x1.append(x1.reshape(x1.size(0), -1))
